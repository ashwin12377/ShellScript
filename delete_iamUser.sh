...
# Prompt for the IAM username
read -p "Enter the IAM username 🤡: " BASE_USER_NAME
USER_NAME="${BASE_USER_NAME}"

# Check if the IAM user exists and get the exact username
EXACT_USER_NAME=$(aws iam get-user --user-name $USER_NAME --query 'User.UserName' --output text 2>/dev/null)
if [ -z "$EXACT_USER_NAME" ]; then
    echo "IAM user '$USER_NAME' does not exist❌"
    exit 1
fi

# Verify case-sensitive username match
if [ "$USER_NAME" != "$EXACT_USER_NAME" ]; then
    echo "IAM user '$USER_NAME' does not match the exact case-sensitive username '$EXACT_USER_NAME'❌"
    exit 1
fi
